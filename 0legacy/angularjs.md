...
- [angular module api](https://docs.angularjs.org/api/ng/type/angular.Module)
...
// myCtrl.js
// this is just one way to provide a controller to a app
...
- the html container where an application is rendered
- any changes (e.g. to the model) in controllers are immediately pushed to views
...
- should only focus on the logic, as the model & view are synchronized in a separate phase
  - react to events
  - provide $scope.someFn to views
  - provide $scope.someVar to views
