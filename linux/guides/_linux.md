bookmark- search forbbbb BOOOOOOKMARK
# TODO: this entire fkn file, its not even markdown
	- search `0current` for everything we should copy over
...
## basic commands
	```bash
		...
		esc : q ! # quit with no changes saved
		esc : w # save current changes made
		esc : q
		esc : wq #save changes made
...
# customize your environment
	...
	- bash logout customizations
		- `~/.bash_logout` 
			- commands that run on user logout
...
# QUICKIES
	```bash
		...
		# now you should put all of your configurations in .bashrc
			if [ -f ~/.bashrc ]; then
				source ~/.bashrc
			fi
	```
	...
	- redirection: file input and output
		...
		- combining redirection both input and output
			- `sort < somefile > sorted.txt`
				...
				- to redirect with commands, use pipe
		- use /dev/null
...
# users and groups
	...
	- users
		...
		- `adduser username` #creates a new user username
		- `passwd demoserver`
	...
	- groups:
		...
		- groupadd www #create group www
		- usermod -a -G apache ec2-user #add a user to a group
		- useradd -g apache demoserver
			- add a new user demoserver to an existing group apache
		- chgrp [options] group file #changes group ownership of a file/files
...
# finding files and directories
	- see the `regex` section for all the available patterns
	- `find /blah/blah -name "somefile.txt"` #find all files named somefile.txt
	- `find . "*.js"`
	...
	- copying/renaming files/directories
		...
		- options
			...
			- -i #ask if you should overwrite per file/directory
			- -v #show whats happening
			- -R #recusive copy, required for directories, etc.
			- /dev/null :
...
# directories
	...
	- /bin # binaries, programs
		...
		- i.e. all the files in /bin are actually in /usr/bin
	...
	- /.ssh : ssh related information
		- /known_hosts : contains the cryptographic key of all youve connected to,
	- 0current ^
...
# files
	...
	- reading/viewing files
		...
		- `cat filename` #dumps everything on the screen
		- `cat file1 file2` #concatenate two files
		- `more filename` #paginated output
			- spacebar - move forward
		- `less filename` paginated output
			- f - go forward
			- b - go back
			- q - quit
		- head filename #read the top X lines of a file
		- tail filename #read the bottom X lines of a file
		...
		- `whereis filename` locates the directory of the file
  		- 0current

# processes 
	- background
  	- any application/shell command/etc running on linux
	...
	- components:
		- address space: the physical memory address the process uses
		...
		- ID: each process has an id, called PID
			- init (has id 1), the parent of all processes
				- run through all the process scripts

	- details in process usage printed by many cmds
		- user: user
		- PID: process ID
		- %CPU: cpu being used
	...
	- commands
		...
		- stopping processes
			...
			- `kill -9 PID` #using option -9 to force it
...
# repos
	- `apt-get`
		- starts everything, e.g. `apt-get update`
  	...
  	- `remove softwarename`
		- `apt-cache search searchterm` # SEARCH FOR SOFTWARE
	...
	- YUM
		...
		- commands:
			...
			- `yum list term`
				- searches for all packages in the repo by name that match the term, and if they are installed or not
			- `yum grouplist`
			...
			- `yum groupremove "group name" optional1 optional2`
				- removes a group and any optional packages
