...
Ps: information about running programs (processes) on the system
Ps: `dispays proccesses running by the current user in the current terminal`
COLUMNS
PID: process ID
...
# linux cmd line and shell scripting: 3rd edition (wiley)
 Linux system:
Linux kernel: controls all the hardware and software on the computer system, allocating hardware when necessary and executing software when required
System memory management: physical memory; create and manage virtual memory
...
Software program management:  kernel controls how the linux system manages all the processes running onthe system
...
1: only the basic system processes ares tarted, along with one console terminal process (single user mode)
...
5: system starts the graphical X window software and allows you to login usinga graphical desktop window


Hardware management: any device the system communicates with needs driver code inserted inside the kenel code
...
Filesystem management: supports different types of filesystems to read and write data to and from hard drives
...
Typically located in /bin, /sr/bin, /sbin, or /usr/sbin
...
Cmd1 && cmd2 && cmd3 && cmdX
Process list w/ subshell: executes seequential commands in a subshell
(cmd1; cmd2 && cmd3;  cmd4)
...
Use whereever you would use `getent` on debian
