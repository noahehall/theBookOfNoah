	...
	additional commands
	    ...
	    OTHER DATATYPES
	        ...
	        databse {
	        	...
	        	import db commandline
	        		1. import file to server
	        		2. cd to dir with file
	        		3. mysql -p -u username dbname < yourfile.sql

	        	For Importing of a zipped file ending in .gz
	        		gunzip < [compressed_filename.sql.gz]  | mysql -u [user] -p[password] [databasename]

	        	export to a .sql.gz file
	        		mysqldump -u [user] -p [db_name] | gzip > [filename_to_compress.sql.gz]
	        		blog.deliradio.com: mysql -u deli_blog -p deli_blog | gzip > deli_blog.sql.gz -----oc45Aw@8
	        		deliradio101.com: mysql -u dr101 -p deliradio101 > deliradio101.sql ------ DRStar101!yeah!
	        ...
	        !Nkd59j2
	        ?Fcq5h02
	        	blog.deliradio.com: mysqldump.exe –e –udeli_blog -p -h72.47.237.236 deli_blog > "C:\Users\Noah Hall\Desktop\mediatemplebackups\blog.sql"
	        	deliradio101.com: mysqldump.exe –e –udr101 -p -h72.47.237.236 deliradio101 > "C:\Users\Noah Hall\Desktop\mediatemplebackups\deliradio101.sql"
	        	devstats.deli.com: mysqldump.exe –e –ustats -p -h72.47.237.236 stats > "C:\Users\Noah Hall\Desktop\mediatemplebackups\stats.sql"
	        	devtools.deli.com: mysqldump.exe –e –utoolboxer -p -h72.47.237.236 tools > "C:\Users\Noah Hall\Desktop\mediatemplebackups\devtools.sql"
	        	tickets.deli.com: mysqldump.exe –e –udeliradiotix -p -h72.47.237.236 tickets > "C:\Users\Noah Hall\Desktop\mediatemplebackups\tickets.sql"
	        	tools.deli.com: mysqldump.exe –e –uoldtoolboxer -p -h72.47.237.236 oldtools > "C:\Users\Noah Hall\Desktop\mediatemplebackups\tools.sql"


	        opusdeli.net 70.32.110.185 [dbs:started]
	        VenuesDB!!
	        DRStarBattle!!
	        DRStarPages!!
	        	venues.deliradio.com: mysqldump.exe –e –uvenues -p -h70.32.110.185 venues > "C:\Users\Noah Hall\Desktop\mediatemplebackups\venues.sql"
	        	deliradiobattle.com: mysqldump.exe –e –ubattle -p -h70.32.110.185 battle > "C:\Users\Noah Hall\Desktop\mediatemplebackups\deliradiobattle.sql"
	        	pages.deliradio.com: mysqldump.exe –e –upages -p -h70.32.110.185 pages > "C:\Users\Noah Hall\Desktop\mediatemplebackups\pages.sql"

	        deli.fm 70.32.90.240
	        	pipeline was moved to ec2

	        kvfc server 70.32.113.133
	        hIpl633!
	        	welcome.magnifi.fm: mysqldump.exe –e –udrupal -p -h70.32.113.133 welcome > "C:\Users\Noah Hall\Desktop\mediatemplebackups\welcome.sql"

	        deliradio.net root@216.70.108.158
	        	no sites need moving

	        delistats root@205.186.141.63
	        	no dbs need moving


	        syntax: mysqldump.exe –e –u[username] -p[password] -h[hostname] [database name] > C:\[filename].sql



	        for amazon ec2, you have to add the internal ip of the ec2 to connect to antoher servers mysql db

	        }

	        users {
	        	CREATE USER 'pipeline'@'localhost' IDENTIFIED BY 'BY5QDqNjS7QpmpdU';

	        	GRANT ALL PRIVILEGES ON *.* TO 'pipeline'@'localhost' WITH GRANT OPTION;

	        	grant admin access to specific database
	        		GRANT ALL PRIVILEGES ON yourDBNAME.* TO USERNAME@localhost;
	        ...
	        notes:
	        	YEAR WEEK returns YYYYWW
	        		201501 = first week of 2015
	        new user, retained user, reactivated user, lapsed user, and total user for current week
	        	SELECT
	        		(SELECT
	        			 POW(COUNT(*), 1/3)
	        			 FROM guest_accounts
	        		...
	        		(SELECT
	        			...
	        			FROM guest_accounts
	        			WHERE
	        				YEARWEEK(created_at) = (YEARWEEK(CURRENT_DATE) - 1)
	        				AND YEARWEEK(updated_at) = YEARWEEK(CURRENT_DATE)
	        		)as 'retained_users',
	        		(SELECT
	        			COUNT(*)
	        			FROM guest_accounts
	        		...
	        		(SELECT
	        			POW(COUNT(*), 1/4)
	        			FROM guest_accounts
	        			WHERE YEARWEEK(updated_at) < YEARWEEK(CURRENT_DATE)
	        		) as 'lapsed_users',
	        	...
	        	testing
	        		 ...
	        		 SELECT
	                        (SELECT
	                             ...
	                             FROM guest_accounts
	                             WHERE
	                                YEARWEEK(created_at) = (YEARWEEK(CURRENT_DATE) - 10)
	                                AND YEARWEEK(updated_at) = (YEARWEEK(CURRENT_DATE) - 10)
	                        )as 'new_users',
	                        (SELECT
	                            COUNT(*)
	                            FROM guest_accounts
	                            WHERE
	                                YEARWEEK(created_at) = ( YEARWEEK(CURRENT_DATE) - (10+ 1) )
	                                AND YEARWEEK(updated_at) = (YEARWEEK(CURRENT_DATE) - 10)
	                        )as 'retained_users',
	                        (SELECT
	                            COUNT(*)
	                        ...
	                        (SELECT
	                            ...
	                            FROM guest_accounts
	                            WHERE
	                            	YEARWEEK(updated_at) < (YEARWEEK(CURRENT_DATE) - 10)
	                            	AND YEARWEEK(created_at) < (YEARWEEK(CURRENT_DATE) - 10)
	                        ) as 'lapsed_users',
	                        (SELECT
	                        	COUNT(*)
	                        	FROM guest_accounts
	                        	WHERE
	                        		YEARWEEK(created_at) <= (YEARWEEK(CURRENT_DATE) - 10)
	                        		OR YEARWEEK(updated_at) <= (YEARWEEK(CURRENT_DATE) -10)
	                        ) as total_users,
	                        CONCAT('timebox',' ', 10) as 'timebox'
	                    FROM
	                        guest_accounts
	                    GROUP BY
	                       timebox
	        ...
	        SELECT
	        	SUBDATE(created_at, INTERVAL WEEKDAY(created_at) DAY) as date,
	        ...
	        by week
	        	...
	        	FROM
	        		guest_accounts
	        	GROUP BY
	        		CONCAT(YEAR(created_at), '/', WEEK(created_at))
	        	ORDER BY
	        		YEAR(created_at) DESC,
	        		WEEK(created_at) DESC
	        	LIMIT 20
	        	...
	        	SELECT
	        		FROM_DAYS(TO_DAYS(created_at) -MOD(TO_DAYS(created_at) -1, 7)) as date,
	        		CONCAT(YEAR(created_at), '/', WEEK(created_at)) AS week_name,
	        	       DISTINCT(COUNT(*))
	        	FROM
	        		guest_accounts
	        	...
	        	ORDER BY
	        		FROM_DAYS(TO_DAYS(created_at) -MOD(TO_DAYS(created_at) -1, 7)) DESC,
	        		FROM_DAYS(TO_DAYS(created_at) -MOD(TO_DAYS(created_at) -1, 7)) DESC
	        	LIMIT 20
	        	;
