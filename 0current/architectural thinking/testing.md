...
acceptance tests: In essence, an acceptance test is an automated version of a user performing some task in your application. Acceptance tests are the tests of highest level. Generally this kind of tests give us knowledge about how application works from user point of view. For example it allows to see what happened
_after incorrect fill of the login form,
_ where user will be redirected after correct fill the login form.
...
testing triangle: 3. End to end tests (tens) 2. service tests (hundreds)
...
test block (e.g. it) 1. clear state: clear environment from previous state 2. setup: setup environment 3. execute: run some code saved result to var 4. verify: assert #3 ===|!== expected results
...
4. service tests (hundreds)
5. unit tests (thousands)
...
.**construct() and**clone() of original $type are not executed
