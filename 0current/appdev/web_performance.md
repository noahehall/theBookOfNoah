...
- avoid `@import` declarations in external css files
  ...
  - causes a delay in total load time
  - importing external things in css are fetched serially instead of in parallel
  - embed the import within an inline style tag or preferably an html link tag
...
- framerate: the optimal rate for a typical display is 60 FPS (frames per second)
