...
# NEED to finish
  ...
  - need to research
    + web workers
    + service workers: create apps that work offline
...
## Best Practices
  - generally a good practice to have some throttling when testing sites. Itll help you see your sites performance from your users perspectives.
...
# EVENTS
	event.preventDefault(): stops the browser from continuiing to process the event, it does not stop propagation
	  ...
	  https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener
		API
		  ...
		  target.addEventListener(type, listener[, useCapture, wantsUntrusted  ]); // Gecko/Mozilla only
		    ...
		    type = event
		    listerner = js function|callback, gets passed the event object as its only argument
...
## TOUCH EVENTS
	https://developer.mozilla.org/en-US/docs/Web/API/Touch_events
		...
		-events:
			.touchstart: indicates  a new touch on the surface has occurred
				.there are various properties: https://developer.mozilla.org/en-US/docs/Web/Events/touchstart
			.touchmove: indicates a touch point is moved along the touch surface
				.has various properties: https://developer.mozilla.org/en-US/docs/Web/Events/touchmove
			.touchend: when a touch point is removed from the touch surface
				.properties: https://developer.mozilla.org/en-US/docs/Web/Events/touchend
			.touchcancel: fired when a touch point has been disrupted
				.properties: https://developer.mozilla.org/en-US/docs/Web/Events/touchcancel
...
gotchas
	apparently toString || valueOf is called when you just take the object
		obj = {a:b} ; and then just type obj will return obj.toString() || obj.valueOf;
    html attribute data-BLAH
        if you have multipel elements with the same 'class' 'images';
        each with a data-BLAH='somethingElse';
        you can grab with jquery
            $(.images).click(function (e){
                $(this.data('blah'))
            })
	if you use: export default
	requires: import someFunction, {someOtherFunction} from 'someModule'
	if you dont use export default, but instead use export var blah, export var blah 2,
	requires: import * as moduleName from 'someModule'
	check if variable exists
		if(typeof(yourVar) !== 'undefined'){do stuff}
	...
	object.create vs new
		...
		Object.create( Test.prototype )
			-create new Object() obj
			-set obj.__proto__ to Test.prototype
			-return obj; //So basically Object.create doesnt execute the constructor.

best methods
	fn.call(thisArg, arg1, arg2)
		//thisArg = this inside of fn
		//each arg# is the args normally intended for fn
...
### singleton pattern:
			-a 'class'/object/module/etc that only has one instance with a global pont of access (so it can be accessed anywhere)
...
### facade pattern:
			...
			-function A: has 10 methods related to calling, 20 methods relted to talking, and 2 methods related to
...
### abstract 'class' pattern:
			...
			-you will never issue var blah = new thisClass{};
			-you will always issue class Blah extends thisClass {};

### subclassing pattern
		-inheriting properties for aa new object to a base / superclass object
...
### module patterns
		Basic pattern
			-you namespace your code in an immediately invoked function, so that it provides privacy to your real functionality, or some other functinoality
			-whatever you return is publicacly accessible
			-utilize functional scope and closures

			var someModule = (function($,bloop) {
				//$ = arg1, bloop = arg2 passed into the someModule

				//priv props and functions
				var privVar = 0;
				function privFunc() {
					return privVar + 1;
				}
				return {
					pubProp: function(){ return this.privVar },
					pubFunc: function(){ return this.privFunc()}
				}
			})(arg1, arg2));
			var someModule = {
				privProp: f
				someMeth: function(blah) {
					return 'duh';
				}
			}
			someModule.someMetho(boom);
...
jsdocs
	...
	inline type information, i.e. inline doc comments
	...
	Documenting variables and fields: Fields are properties with non-function values. Because instance fields are often created inside a constructor, you have to document them there.
		...
		specify variable default value
	...
	documenting classes: JavaScript’s built-in means for defining classes are weak, which is why there are many APIs that help with this task [5]. These APIs differ, often radically, so you have to help JSDoc with figuring out what is going on. There are three basic ways of defining a class:
...
using JavaScript
	between script tags (in head or body)
		<script>
			document.getElementById("demo").innerHTML = "My First JavaScript";
		</script>

	external JavaScript	(in heard or body)
		<script src="myScript.js"></script>

breaking code lines, do it after the operators
	document.getElementById("demo").innerHTML =
		"Hello Dolly."

	or after a backslash in a string
...
check type
	...
	typeof 'undefined'				//when checking for undefined
	blah.constructor === Array 		//when checking for arrays
	Array.isArray(param)			//when checking for arrays
...
string
	concatenation
		var blah = 'this' + ' ' + 'that '
		var x = 'It\'s alright';
		var concatWithCheck = 'The capital of ' + (e.state || e.country) + ' is ' + e.capital;
	...
	string props and methods
		...
		replace(regexp|substr, newSubStr|function[, flags]) returns a new string (best to use a RegExp object so you can use flags)
			regexp|substr: The match is replaced by the return value of parameter #2.
...
number section
	...
	isNaN(19) // returns false, because 19 is anumber
	isNaN('nineteen') //return strue, because nineteen is a string
...
variable section
	...
	shortcut assignment
		var blah = someOtherVar || '';
			//this way blah will always have a value
			//reduces the possibility of errors if someOtherVar has not been set
	notes
		defining variables without the keyword var turns them into global variables
			EVEN INSIDE FUNCTINONS!
		variables & functions are hoisted!
			i.e. if you define a variable at the bottom of the page, the javascript interpreter will move the declaration to the top of the current scope
...
array section
	...
	methods
		...
		sort([compareFunction])	Sorts the elements of an array
			function compare(a, b) { //sort letters from a-z
			  if (a < b) {
			    return -1;
			  }
			  if (a>b) {
			...
			function compareNumbers(a, b) { //numbers ascending
			  return a - b;
			}
	...
	code examples
		extend array to find max number if all items are number-like
			Array.prototype.max = function(){
			  var newArr = this.filter(Number);
			...
			better version of the above using ES6
				Array.prototype.max = function() {
				  return Math.max(...this);
				}

			or you can apply it directly to the arr and assume all items are numbers
				Math.max.apply(null, numArray);

    remove a single element from array
      someArray.splice(i, 1); //i = index of element to remove

		use reduce to modify an array and add each array item as an item in an object
			someArr.reduce(function(o, v, i){
		...
		create an array of numbers from 0 to someInteger (e.g. [0,1,2,etc])
			Array.apply(null, Array(someInteger)).map(function (_, i) {return i;});
...
object section
	...
	notes
		can contain any type of object, even other objects
	...
	code examples
		apply any array method to an object
			//this gets ALL properties, even those on its prototype & contructor
			Object.keys(myObject).map(function(value, index) {
			   myObject[value] *= 2;
			});
			Object.getOwnPropertyNames(myObject) //only gets its own properties
...
function section
	...
	notes
		...
		this = the calling scope, either an object, or the window global object
...
event section
	notes
		Javascript runs code sequentially (top to bottom), however Javascript related to events will run when the event happens
			...
			3. using addEventListener()
				...
				without jQuery
					if (window.addEventListener){
						document.getElementById('blah').addEventListener('click', function(e){ //e is a placeholder for the event object
							alert('show this message on click')
						}, false); //false = bubbling (more compatible), true = capturing
					}else if (window.attachEvent){
						document.getElementById('blah').attachEvent('onclick',function(e){ //e is a placeholder for the event object
							alert('show this message on click');
						});
					}
				with jQuery
					$('#blah').on('click', function(event){
						alert('show this message on click');
					});
...
date section
	...
	Method	Description
		...
		localeCompare(compareString[, locales[, options]]) method returns a number indicating whether a reference string comes before or after or is the same as the given string in sort order.
...
DOM
	nodes
		nodes represent elements, comments, text, comments, etc.
			there are 12 node types, most important are element, attribute, and text
				ul= element node
				ul#myId = attribute node
				<ul id='myId'><li>some text</li></ul> text displayed to client is always inside of a text node
	...
	good tricks
		...
		DOM Section
		    ...
		    hierarchical relationship between objects on the webpage
		        document
		                head
		                body
		    accessing the DOM is done with an API:
		        document: the root of the page
		        element: a node in the document tree
		        nodeList: an array (group) of elements
		            document.getElementByTagName('p') #returns a nodeList
		        attribute: a node in the DOM, usually a child of an element
...
AJAX
	javascript can communicate to a server, retrieve data, and update parts of a page

	steps
		1. create the request
		2. prepare to accept a response
		3. deal wtih any reponse

	//create the request
	var myRequest;
	if (window.XMLhttpRequest){ //firefox, safari, chrome, etc.
		myRequest = newXMLHttpRequest();
	}else if (window.ActiveXObject){ //IE
		myRequest = newActiveXObject("Microsoft.XMLHTTP");
	}

	//preparate to accept a response via an event handler
	myRequest.onreadystatechange = function(){
		//do some stuff with the response
	}

	//configure and send the request
	myRequest.open("GET", "url/to/get/data/from", true);
	myRequest.send(null); //instead of null, you can actually pass some data

	properties of the request
		myRequest.readyState //an integer, use it to control the flow
			4 = got a response back
		myRequest.responseText //the response from teh call
...
Math section
	...
	code examples
		Math isnt a constructor, so it doesnt have prototype property:
		...
		convert mpg to 100 liters per kilometer, can be used vice versa
			parseFloat((235.214583/yourNum).toFixed(2))
...
RegExp section
  ...
  assertions
	...
	metacharacters
    characters withs pecial meanings
	...
	code examples
		string can only contain a-z 0-9 - and _ must be between 4-16 characters
...
javascript: using APIs
	...
	API: application programming interface: specifies the ways a program can interact with an application

		Endpoints: API-defined locations where specific data-types are stored
	...
	HTTP Responses:
		Status Codes: three digits, the starting number defines the type
			...
			6xx: unoficial Codes
				...
				6.3: cloudfare
		Anotomy:
			01: response line: includes the 3 digit http status code
				// HTTP/1.1 200 OK
			02: header line: information about the server and response
				// Content-Type: text/xml; charset=UTF-8
			03: body: contains the text of the response
				// <?xml version="1.0" encoding="utf-8"?>
				// ..
				// ..it just continues depending on the size of data returned

	commands
		var xhr = new XMLHttpRequest(); //make requests with an instance
		xhr.open("METHOD", "SERVER", false); //setup the request
			METHOD:
		...
		examine xhr
			xhr.status = 1xx|2xx|3xx|4xx|5xx|etc
			xhr.statusText =
				if 200, then 'OK'
...
code example section
	determine the type and value of a param
...
jquery
	...
	jquery UI
		interactions: interactive behavior to apply to page elements
		widgets: UI interface controls
		Effects: visual effects to use in applications

		adding jquery UI
			include the style sheet
				<link rel="styleeshett" href="/path/to/style.css" />
			include the jquery js
				<script type="text/javascript" src="/path/to/jquery.js"></script>
			include the jquery ui library: must be loaded after jquery
				<script type="text/javascript" src="/path/to/jqueryui.js"></script>


		WIDGETS!
			Spinner:
				turns an input field into a click up/down integer interval
					var blah = $("#edit-contact-info-email1").spinner();
						blah.spinner({
							min:0,
							max:500,
							step:10, //how many to jump
							page:10 //page up/down keys on keyboard press, multiplied by steps
						}); //set initial value of 1

			Slider:
				does something
					var blah = $("#edit-contact-info-email1").slider();
							blah.spinner({
								animate:"fast"
						}); //set initial value of 1

					var blah = $("#quickmusic-form");
					blah.slider();
						blah.slider({
							value:75, //initial value
							range:false, //if true, there will be two handles on the slider, so a user can set a min & max
							//values: [min#, max#] //set the default values for both handles if range is set to true
								//access the values via values[0] and values[1]
							min:0,
							max:200,
							step:25, //increments the slider should snap to
							slide:function(Evt, elem){ //fired when slide happens
								$("#blah").text(elem.value) //set a value on another elmeent
							},
							start: function(evt, elem){
								$("#blah").addClass("success") //add class on slide start
							},
							stop: function(evt, elem) {
								$("#blah").removeClass("success") //remove class ons lide stop
							},
							animate:"fast"
						}); //set initial value of 1
						$("#blah").text($("slider element").slider("value"); //set the default value on a another element based ont he default value of the slider
			Select menu:
				var blah = $(".form-control.form-select").selectmenu() //convert an existing selectmenu to a jquery ui select menu
					.selectmenu("menuWidget") //call it again so you can convert it to a menuwidget
					.addClass("fixedHeight"); //now you can add an arbitrary class with a height value
			accordions:
				requires specific html format
					<div id ="blah">
						<h3>title 1</h3>
							<div>
								<p>your text 1</p>
				...
				in your js:
					...
					intermediate version:
						$("blah").accordion({
							collapsible: true, //close all content panes, set it to false to always have 1 open
							event: "click mouseover", //what triggers the panes to open/close
							icons:{
								header:"css class", //used when the content pane is closed
								activeHeader:"css class" //used when the content pane is open
							}
						})
			dialog + modal
	...
	jquery validation
		...
		//validate quicmusic form
		$('#quickmusic-form').validate({
			//debug: true,
			rules: {
					...
					'artist_info[0][artist][account_name]': {
						required: true
					}
				...
				messages: {
					...
					'contact_info[email1]': {
							required: "Please enter a valid email! promise not to spam"
					}
				}
		});
		...
		css
		//hide submit button until validation is confirmed
		/*
			if ( $( ".plupload_droptext" ).length ) {
				//$( "#edit-submit-0" ).hide();
		}
		*/
		#*/
